...
fn generate_std_mapping(write: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if write {
        ...
        let start_marker = "static STD_MODULE_MAPPING: Lazy<HashMap<&'static str, &'static str>> = Lazy::new(|| {";
        let end_marker = "});\n";
        
        let start_pos = current_content.find(start_marker)
        ...
        let relative_end = current_content[search_from..].find(end_marker)
        ...
        let doc_start = current_content[..start_pos].rfind("/// Mapping of std library modules")
