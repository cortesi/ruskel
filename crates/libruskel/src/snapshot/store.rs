...
use crate::{
    ...
    snapshot::{
        ApiSnapshot, SnapshotProfile,
        manifest::{MARKER_FILENAME, Manifest, expected_header},
    },
};
...
fn validate_tree(
    ...
    for entry in &manifest.crates {
        ...
        if !text.starts_with(&expected_header(entry)) {
            return Err(store_error(
                &file,
                "managed file header does not match marker identity",
            ));
        }
    }
    Ok(TreeState {
...
mod tests {
    ...
    fn crate_snapshot(package: &str, crate_name: &str, body: &str) -> CrateSnapshot {
        CrateSnapshot {
            ...
            contents: format!(
                "{}pub mod {crate_name} {{\n    {body}\n}}\n",
                expected_header(&super::super::manifest::ManifestCrate {
                    package: package.into(),
                    crate_name: crate_name.into(),
                    file: format!("{package}.rs"),
                })
            ),
        }
    }
    ...
    fn malformed_unsupported_collision_and_header_mismatch_are_rejected() -> Result<()> {
        ...
        fs::write(
            ...
            captured.crates()[0]
                .contents()
                .replace("// crate: alpha", "// crate: other"),
        )?;
        assert!(SnapshotStore::open(&output, SnapshotMode::Check).is_err());
        Ok(())
    }
